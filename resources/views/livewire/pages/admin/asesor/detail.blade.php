...
<x-ui.page title="Detail Asesor" subtitle="Profil lengkap, rekam jejak, dan dokumen pendukung asesor.">
        ...
        <div class="row g-6">
            ...
            <div class="col-xl-8">
                <div class="d-flex flex-column gap-6">
                                    ...
                                    <x-ui.card title="Karya Publikasi" subtitle="Publikasi dan karya ilmiah yang terkait dengan asesor.">
                                        <div class="px-0 pt-0">
                                            @forelse($asesor->karya_publikasi ?? [] as $item)
                                                <div class="spm-asesor-experience-row">
                                                    <span class="fw-semibold fs-7 text-gray-800 text-truncate pe-3">
                                                        {{ is_array($item) ? ($item['judul'] ?? '-') : $item }}
                                                    </span>
                                                    @if(is_array($item) && !empty($item['link']))
                                                        <x-ui.button :href="$item['link']" target="_blank" variant="light" size="sm" class="btn-active-light-primary">
                                                            <x-ui.icon name="exit-right" class="fs-4" />
                                                        </x-ui.button>
                                                    @endif
                                                </div>
                                            @empty
                    ...
                    <x-ui.section-card title="C. Dokumen Pendukung" subtitle="Dokumen identitas dan sertifikasi yang menunjang profil asesor.">
                        <div class="p-6">
                            @php
                                $asesorDocs = [
    ...
    @else
        <x-ui.card>
            <x-ui.empty-state
                title="Profil Kosong"
                description="Asesor ini belum melengkapi profil mereka."
            />
        </x-ui.card>
    @endif
</x-ui.page>
