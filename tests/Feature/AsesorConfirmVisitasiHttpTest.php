...
class AsesorConfirmVisitasiHttpTest extends TestCase
{
    use RefreshDatabase;

    public function test_non_ketua_cannot_confirm_visitasi_selesai(): void
    {
        ...
        $response = $this->actingAs($asesorUser2)->post(route('asesor.akreditasi.confirm-visitasi-selesai'), [
            'akreditasi_id' => $akreditasi->id,
        ]);

        $response->assertSessionHas('error');
        $this->assertSame(AkreditasiStateMachine::STATUS_VISITASI, (int) $akreditasi->fresh()->status);
    }
}
